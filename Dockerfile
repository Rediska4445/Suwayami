...
WORKDIR /src
COPY ["Suwayami/Suwayami.csproj", "Suwayami/"]
RUN dotnet restore "./Suwayami/Suwayami.csproj"
COPY . .
WORKDIR "/src/Suwayami"
RUN dotnet build "./Suwayami.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Этот этап используется для публикации проекта службы, который будет скопирован на последний этап
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Suwayami.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
